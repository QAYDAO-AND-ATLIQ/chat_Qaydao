...
class Captain::Assistant::AgentRunnerService
  ...
  def process_agent_result(result)
    ...
    # QAYDAO: never leak internal runner failures (e.g. "Exceeded maximum turns: N",
    # "Conversation ended: ...") to the customer. Convert to a graceful human handoff.
    if output.is_a?(String) && output.match?(/maximum turns|Conversation ended:/i)
      Rails.logger.warn "[Captain V2][QAYDAO] suppressed internal runner output: #{output}"
      return {
        'response' => 'conversation_handoff',
        'reasoning' => "Suppressed internal runner output: #{output}",
